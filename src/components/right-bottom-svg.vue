<template>
  <svg id="yichang" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="456"
    ...
    <defs>
      ...
      <filter id="hy-filter-2" x="67" y="-6" width="50" height="49" filterUnits="userSpaceOnUse">
        <feGaussianBlur result="blur" stdDeviation="6.667" in="SourceAlpha" />
        <feComposite result="composite" />
        <feComposite result="composite-2" />
        <feComposite result="composite-3" />
    ...
    <!-- 多个点 -->
    <template v-for="(dot, index) in props.dots" :key="index">
      <circle class="hy-cls-3" :cx="dot.x" :cy="dot.y" r="5">
        <animateMotion :dur="dot.dur" repeatCount="indefinite" rotate="auto" :begin="dot.begin">
          <mpath href="#curve"></mpath>
        </animateMotion>
      </circle>
    </template>
